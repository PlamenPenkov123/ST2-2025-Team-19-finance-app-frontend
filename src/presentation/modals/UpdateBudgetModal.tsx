...
export const UpdateBudgetModal = (props: {
    ...
    const onSubmit = async () => {
        ...
        if (!amount() || !date()) {
            setError("Please provide both amount and date.");
            return;
        }
        ...
        try {
            ...
            // Ensure proper YYYY-MM-DD format (date input already gives this, but we’ll be safe)
            const formattedDate = new Date(date()!).toISOString().split("T")[0];

            await repo.updateBudget(authToken, Number(amount()), formattedDate);
            props.onSuccess();
            props.onClose();
        } catch (err) {
            console.error(err);
            setError("Error while adding budget. Please try again.");
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <Modal state={props.state} onClose={props.onClose}>
            {() => (
                <div class="max-h-[90vh] overflow-y-auto w-[90%] sm:w-[28rem] bg-white border border-[#E2E8E2] rounded-2xl shadow-lg p-6 sm:p-8 relative text-gray-800">
                    <LoadingIndicator isLoading={isLoading()} loadingText="Saving budget..." />
                    ...
                    >
                        ...
                        <p class="text-sm text-gray-500 text-center mb-3">
                            Set your spending goal for a specific date with{" "}
